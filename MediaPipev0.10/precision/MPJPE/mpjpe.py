...
class MPJPE:
    def __init__(self, basepath, images_path, labels_path):
        ...
        # Mapeo de keypoints de MediaPipe
    ...
    def __get_inference(self, image_path, true_keypoints, image, threshold, results):
        """
        Realiza la inferencia con MediaPipe y calcula el PCK para una imagen.
        :param image_path: Ruta de la imagen
        :param true_keypoints: Coordenadas verdaderas de los keypoints
        ...
        results_mp = self.pose.process(img_rgb)
        
        if results_mp.pose_landmarks:
            # Inicializar array para los keypoints predichos (17 puntos)
            pred_keypoints = np.zeros((17, 2))
            
            # Extraer los keypoints según el mapeo definido
            for idx, landmark in self.keypoint_mapping.items():
                landmark_data = results_mp.pose_landmarks.landmark[landmark]
                pred_keypoints[idx, 0] = landmark_data.x
                pred_keypoints[idx, 1] = landmark_data.y
            ...
            if mpjpe is not None:
                ...
                print(f"No se pudieron calcular métricas para la imagen {image} (ningún punto visible).")
        else:
            print(f"No se detectaron pose en la imagen {image}.")
